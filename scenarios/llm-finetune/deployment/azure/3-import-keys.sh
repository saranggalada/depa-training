...
# Import encryption keys for each party's data + model + output into AKV,
# bound to a key release policy with the CCE policy hash.
...
# Obtain bearer token based on the AKV resource endpoint subdomain
...
import_key "MarketDataEncryptionKey" $DATADIR/market_data_key.bin
import_key "FintechEncryptionKey" $DATADIR/fintech_key.bin
import_key "BankEncryptionKey" $DATADIR/bank_key.bin
import_key "ModelEncryptionKey" $MODELDIR/model_key.bin
import_key "OutputEncryptionKey" $MODELDIR/output_key.bin
