...
function generate_encrypted_filesystem_information() {
  ...
  MARKET_DATA_SAS_TOKEN=$(az storage blob generate-sas --account-name $AZURE_STORAGE_ACCOUNT_NAME --container-name $AZURE_MARKET_DATA_CONTAINER --permissions r --name data.img --expiry $end --only-show-errors)
  export MARKET_DATA_SAS_TOKEN="$(echo -n $MARKET_DATA_SAS_TOKEN | tr -d \")"
  export MARKET_DATA_SAS_TOKEN="?$MARKET_DATA_SAS_TOKEN"

  FINTECH_SAS_TOKEN=$(az storage blob generate-sas --account-name $AZURE_STORAGE_ACCOUNT_NAME --container-name $AZURE_FINTECH_CONTAINER --permissions r --name data.img --expiry $end --only-show-errors)
  export FINTECH_SAS_TOKEN=$(echo $FINTECH_SAS_TOKEN | tr -d \")
  export FINTECH_SAS_TOKEN="?$FINTECH_SAS_TOKEN"

  BANK_SAS_TOKEN=$(az storage blob generate-sas --account-name $AZURE_STORAGE_ACCOUNT_NAME --container-name $AZURE_BANK_CONTAINER --permissions r --name data.img --expiry $end --only-show-errors)
  export BANK_SAS_TOKEN=$(echo $BANK_SAS_TOKEN | tr -d \")
  export BANK_SAS_TOKEN="?$BANK_SAS_TOKEN"

  MODEL_SAS_TOKEN=$(az storage blob generate-sas --account-name $AZURE_STORAGE_ACCOUNT_NAME --container-name $AZURE_MODEL_CONTAINER --permissions r --name data.img --expiry $end --only-show-errors)
  export MODEL_SAS_TOKEN=$(echo $MODEL_SAS_TOKEN | tr -d \")
  export MODEL_SAS_TOKEN="?$MODEL_SAS_TOKEN"

  OUTPUT_SAS_TOKEN=$(az storage blob generate-sas --account-name $AZURE_STORAGE_ACCOUNT_NAME --container-name $AZURE_OUTPUT_CONTAINER --permissions rw --name data.img --expiry $end --only-show-errors)
  ...
  # [0] Market Data Provider (TDP1)
  TMP=$(jq . encrypted-filesystem-config-template.json)
  TMP=`echo $TMP | \
    jq '.azure_filesystems[0].azure_url = "https://" + env.AZURE_STORAGE_ACCOUNT_NAME + ".blob.core.windows.net/" + env.AZURE_MARKET_DATA_CONTAINER + "/data.img" + env.MARKET_DATA_SAS_TOKEN' | \
    jq '.azure_filesystems[0].mount_point = "/mnt/remote/market_data_provider"' | \
    jq '.azure_filesystems[0].key.kid = "MarketDataEncryptionKey"' | \
    ...
    jq '.azure_filesystems[0].key_derivation.label = "MarketDataEncryptionKey"' | \
    jq '.azure_filesystems[0].key_derivation.salt = "9b53cddbe5b78a0b912a8f05f341bcd4dd839ea85d26a08efaef13e696d999f4"'`

  # [1] Fintech Provider (TDP2)
  TMP=`echo $TMP | \
    jq '.azure_filesystems[1].azure_url = "https://" + env.AZURE_STORAGE_ACCOUNT_NAME + ".blob.core.windows.net/" + env.AZURE_FINTECH_CONTAINER + "/data.img" + env.FINTECH_SAS_TOKEN' | \
    jq '.azure_filesystems[1].mount_point = "/mnt/remote/fintech_provider"' | \
    jq '.azure_filesystems[1].key.kid = "FintechEncryptionKey"' | \
    ...
    jq '.azure_filesystems[1].key_derivation.label = "FintechEncryptionKey"' | \
    jq '.azure_filesystems[1].key_derivation.salt = "9b53cddbe5b78a0b912a8f05f341bcd4dd839ea85d26a08efaef13e696d999f4"'`

  # [2] Bank Provider (TDP3)
  TMP=`echo $TMP | \
    jq '.azure_filesystems[2].azure_url = "https://" + env.AZURE_STORAGE_ACCOUNT_NAME + ".blob.core.windows.net/" + env.AZURE_BANK_CONTAINER + "/data.img" + env.BANK_SAS_TOKEN' | \
    jq '.azure_filesystems[2].mount_point = "/mnt/remote/bank_provider"' | \
    jq '.azure_filesystems[2].key.kid = "BankEncryptionKey"' | \
    ...
    jq '.azure_filesystems[2].key_derivation.label = "BankEncryptionKey"' | \
    jq '.azure_filesystems[2].key_derivation.salt = "9b53cddbe5b78a0b912a8f05f341bcd4dd839ea85d26a08efaef13e696d999f4"'`

  # [3] Model (Modeller's base model)
  TMP=`echo $TMP | \
    jq '.azure_filesystems[3].azure_url = "https://" + env.AZURE_STORAGE_ACCOUNT_NAME + ".blob.core.windows.net/" + env.AZURE_MODEL_CONTAINER + "/data.img" + env.MODEL_SAS_TOKEN' | \
    jq '.azure_filesystems[3].mount_point = "/mnt/remote/model"' | \
    jq '.azure_filesystems[3].key.kid = "ModelEncryptionKey"' | \
    ...
    jq '.azure_filesystems[3].key_derivation.label = "ModelEncryptionKey"' | \
    jq '.azure_filesystems[3].key_derivation.salt = "9b53cddbe5b78a0b912a8f05f341bcd4dd839ea85d26a08efaef13e696d999f4"'`

  # [4] Output (fine-tuned model + logs, read-write)
  TMP=`echo $TMP | \
    jq '.azure_filesystems[4].azure_url = "https://" + env.AZURE_STORAGE_ACCOUNT_NAME + ".blob.core.windows.net/" + env.AZURE_OUTPUT_CONTAINER + "/data.img" + env.OUTPUT_SAS_TOKEN' | \
    jq '.azure_filesystems[4].mount_point = "/mnt/remote/output"' | \
    jq '.azure_filesystems[4].key.kid = "OutputEncryptionKey"' | \
    ...
    jq '.azure_filesystems[4].key_derivation.label = "OutputEncryptionKey"' | \
