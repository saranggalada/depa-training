...
GENOMICS_LAB_SAS_TOKEN=$(az storage blob generate-sas --account-name $AZURE_STORAGE_ACCOUNT_NAME --container-name $AZURE_GENOMICS_LAB_CONTAINER_NAME --permissions r --name data.img --expiry $end --only-show-errors) 
export GENOMICS_LAB_SAS_TOKEN=$(echo $GENOMICS_LAB_SAS_TOKEN | tr -d \")
export GENOMICS_LAB_SAS_TOKEN="?$GENOMICS_LAB_SAS_TOKEN"

PHARMACEUTICAL_COMPANY_SAS_TOKEN=$(az storage blob generate-sas --account-name $AZURE_STORAGE_ACCOUNT_NAME --container-name $AZURE_PHARMACEUTICAL_COMPANY_CONTAINER_NAME --permissions r --name data.img --expiry $end --only-show-errors) 
export PHARMACEUTICAL_COMPANY_SAS_TOKEN=$(echo $PHARMACEUTICAL_COMPANY_SAS_TOKEN | tr -d \")
export PHARMACEUTICAL_COMPANY_SAS_TOKEN="?$PHARMACEUTICAL_COMPANY_SAS_TOKEN"

COMPUTATIONAL_BIOLOGY_LAB_SAS_TOKEN=$(az storage blob generate-sas --account-name $AZURE_STORAGE_ACCOUNT_NAME --container-name $AZURE_COMPUTATIONAL_BIOLOGY_LAB_CONTAINER_NAME --permissions r --name data.img --expiry $end --only-show-errors) 
export COMPUTATIONAL_BIOLOGY_LAB_SAS_TOKEN=$(echo $COMPUTATIONAL_BIOLOGY_LAB_SAS_TOKEN | tr -d \")
export COMPUTATIONAL_BIOLOGY_LAB_SAS_TOKEN="?$COMPUTATIONAL_BIOLOGY_LAB_SAS_TOKEN"

CANCER_INSTITUTE_SAS_TOKEN=$(az storage blob generate-sas --account-name $AZURE_STORAGE_ACCOUNT_NAME --container-name $AZURE_CANCER_INSTITUTE_CONTAINER_NAME --permissions r --name data.img --expiry $end --only-show-errors) 
export CANCER_INSTITUTE_SAS_TOKEN=$(echo $CANCER_INSTITUTE_SAS_TOKEN | tr -d \")
export CANCER_INSTITUTE_SAS_TOKEN="?$CANCER_INSTITUTE_SAS_TOKEN"
...
TMP=`echo $TMP | \
  jq '.azure_filesystems[0].azure_url = "https://" + env.AZURE_STORAGE_ACCOUNT_NAME + ".blob.core.windows.net/" + env.AZURE_GENOMICS_LAB_CONTAINER_NAME + "/data.img" + env.GENOMICS_LAB_SAS_TOKEN' | \
  jq '.azure_filesystems[0].mount_point = "/mnt/remote/genomics_lab"' | \
  jq '.azure_filesystems[0].key.kid = "GenomicsLabFilesystemEncryptionKey"' | \
  ...
  jq '.azure_filesystems[0].key_derivation.label = "GenomicsLabFilesystemEncryptionKey"' | \
  jq '.azure_filesystems[0].key_derivation.salt = "9b53cddbe5b78a0b912a8f05f341bcd4dd839ea85d26a08efaef13e696d999f4"'`

TMP=`echo $TMP | \
  jq '.azure_filesystems[1].azure_url = "https://" + env.AZURE_STORAGE_ACCOUNT_NAME + ".blob.core.windows.net/" + env.AZURE_PHARMACEUTICAL_COMPANY_CONTAINER_NAME + "/data.img" + env.PHARMACEUTICAL_COMPANY_SAS_TOKEN' | \
  jq '.azure_filesystems[1].mount_point = "/mnt/remote/pharmaceutical_company"' | \
  jq '.azure_filesystems[1].key.kid = "PharmaceuticalCompanyFilesystemEncryptionKey"' | \
  ...
  jq '.azure_filesystems[1].key_derivation.label = "PharmaceuticalCompanyFilesystemEncryptionKey"' | \
  jq '.azure_filesystems[1].key_derivation.salt = "9b53cddbe5b78a0b912a8f05f341bcd4dd839ea85d26a08efaef13e696d999f4"'`

TMP=`echo $TMP | \
  jq '.azure_filesystems[2].azure_url = "https://" + env.AZURE_STORAGE_ACCOUNT_NAME + ".blob.core.windows.net/" + env.AZURE_COMPUTATIONAL_BIOLOGY_LAB_CONTAINER_NAME + "/data.img" + env.COMPUTATIONAL_BIOLOGY_LAB_SAS_TOKEN' | \
  jq '.azure_filesystems[2].mount_point = "/mnt/remote/computational_biology_lab"' | \
  jq '.azure_filesystems[2].key.kid = "ComputationalBiologyLabFilesystemEncryptionKey"' | \
  ...
  jq '.azure_filesystems[2].key_derivation.label = "ComputationalBiologyLabFilesystemEncryptionKey"' | \
  jq '.azure_filesystems[2].key_derivation.salt = "9b53cddbe5b78a0b912a8f05f341bcd4dd839ea85d26a08efaef13e696d999f4"'`

TMP=`echo $TMP | \
  jq '.azure_filesystems[3].azure_url = "https://" + env.AZURE_STORAGE_ACCOUNT_NAME + ".blob.core.windows.net/" + env.AZURE_CANCER_INSTITUTE_CONTAINER_NAME + "/data.img" + env.CANCER_INSTITUTE_SAS_TOKEN' | \
  jq '.azure_filesystems[3].mount_point = "/mnt/remote/cancer_institute"' | \
  jq '.azure_filesystems[3].key.kid = "CancerInstituteFilesystemEncryptionKey"' | \
  ...
  jq '.azure_filesystems[3].key_derivation.label = "CancerInstituteFilesystemEncryptionKey"' | \
